...
def get_language_for_country(country_name):
    ...
    try:
        ...
        if country_name.lower() == 'south korea':
            country = pycountry.countries.get(name='Republic of Korea')
        else:
            country = pycountry.countries.get(name=country_name)

        if country and hasattr(country, 'alpha_2'):
            lang_code = country.alpha_2.lower()
            # Special handling for Chinese
            if lang_code == 'cn': lang_code = 'zh-CN'
            if lang_code == 'tw': lang_code = 'zh-TW'
            language_cache[country_name] = lang_code
            return lang_code
    except Exception:
        pass
...
def translate_keywords(keywords, target_language):
    """Translates keywords using a direct API call."""
    if target_language == 'en' or not keywords:
        return []
    ...
    try:
        ...
        return [item['translatedText'] for item in results['data']['translations']]
...
def search_places(query_location):
    ...
    all_places = []
    for term in set(all_search_terms):
        print(f"  Searching for term: '{term}'...")
        url = "https://maps.googleapis.com/maps/api/place/textsearch/json"
        query = f"business named {term} in {query_location}"
        params = {"query": query, "key": API_KEY, "language": target_lang}
...
def get_place_details(place_id):
    ...
    params = {"place_id": place_id, "fields": "name,place_id,formatted_address,geometry,website,international_phone_number,types,address_components,reviews", "key": API_KEY}
    try:
        response = requests.get(url, params=params)
        response.raise_for_status()
...
def get_city_country_from_components(address_components):
    ...
    for component in address_components:
        if 'locality' in component['types']: city = component['long_name']
        if 'country' in component['types']: country = component['long_name']
    return city, country
...
def assign_category(types, name):
    ...
    if any(kw in full_text for kw in ['hotel', 'resort', 'apartments', 'accommodation', 'villa', 'guesthouse', 'lodging', 'inn', 'suites']): return 'Accommodation'
    if any(kw in full_text for kw in ['spa', 'health', 'clinic', 'wellness', 'yoga', 'beauty', 'salon', 'therapies']): return 'Health & Wellbeing'
    if any(kw in full_text for kw in ['design', 'studio', 'gallery', 'art', 'media', 'productions', 'creative', 'photography', 'fashion']): return 'Creative Industries'
    if any(kw in full_text for kw in ['events', 'planning', 'entertainment', 'lounge', 'nightclub']): return 'Event Management'
    if any(kw in full_text for kw in ['real estate', 'properties', 'developments', 'condominium', 'building', 'display village']): return 'Cities & Developments'
    if any(kw in full_text for kw in ['blockchain', 'crypto', 'web3']): return 'Blockchain'
    return 'Other'
...
def main():
    ...
    for loc in locations:
        ...
        for place in places:
            ...
            business = {'place_id': details.get('place_id'), 'name': details.get('name'), 'category': assign_category(details.get('types', []), details.get('name', '')), 'city': city_name, 'country': country_name, 'latitude': lat, 'longitude': lng, 'website': details.get('website', ''), 'phone': details.get('international_phone_number', ''), 'email': '', 'social': '', 'description': first_review, 'verified': False}
            all_business_data.append(business)
    if not all_business_data:
        print("No new business data found.")
    ...
    try:
        ...
        print("curation/locklist.csv not found.")
